...
describe("ItemStack", function()
	...
	describe("to_string", function()
		...
		it("item name, count, wear and meta", function()
			...
			local values, serializedfields = itemstring:match("(%S+%s+%S+%s+%S+)%s+(.*)")
			assert.equals('test 101 1337', values)
			local fields = core.deserialize(serializedfields)
			assert.same({ foo = "bar" }, fields)
		end)
